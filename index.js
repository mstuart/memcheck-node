...
export default async function memcheck(scenario, options = {}) {
  ...
  for (let i = 0; i < 3; i += 1) {
    // biome-ignore lint/performance/noAwaitInLoops: Warmup scenarios must run sequentially.
    await scenario();
  }
  ...
  for (let i = 1; i <= iterations; i += 1) {
    // biome-ignore lint/performance/noAwaitInLoops: Snapshots require sequential scenarios.
    await scenario();
  ...
  const growth =
    firstSnapshot.usedHeapSize === 0
      ? 0
      : (lastSnapshot.usedHeapSize - firstSnapshot.usedHeapSize) /
        firstSnapshot.usedHeapSize;
