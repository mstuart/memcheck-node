...
import memcheck, { assertNoLeaks, formatReport } from "./index.js";
...
test("memcheck snapshots array has correct length", async (t) => {
  const iterations = 4;
  const result = await memcheck(noop, { iterations });
...
test("memcheck snapshots have correct iteration numbers", async (t) => {
  const result = await memcheck(noop, { iterations: 3 });
...
test("memcheck snapshots have usedHeapSize", async (t) => {
  const result = await memcheck(noop, { iterations: 2 });
...
test("memcheck uses default options", async (t) => {
  const result = await memcheck(noop);
  t.truthy(result);
...
test("formatReport shows LEAK DETECTED for leaked result", (t) => {
  const fakeResult = {
    ...
    leaked: true,
    snapshots: [
      { iteration: 0, totalHeapSize: 2_000_000, usedHeapSize: 1_000_000 },
      { iteration: 1, totalHeapSize: 2_000_000, usedHeapSize: 1_250_000 },
    ],
  };
...
test("assertNoLeaks returns result for clean scenario", async (t) => {
  const result = await assertNoLeaks(noop, { iterations: 2 });
  t.truthy(result);
  t.is(typeof result.leaked, "boolean");
...
test("memcheck with single iteration", async (t) => {
  const result = await memcheck(noop, { iterations: 1 });
